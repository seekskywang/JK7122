...
void FSMC_LCD_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		   //最大速度10MHz
...
/****************************************************************************
* 名    称：void FSMC_RAM_Init(void)
* 功    能：基于FSMC接口的16位RAM接口初始化
* 入口参数：无
* 出口参数：无
* 说    明：
* 调用方法：无 
...
//  
  ...
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;		   //最大速度10MHz
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;		   //复用模式
  GPIO_Init(GPIOD, &GPIO_InitStructure);
